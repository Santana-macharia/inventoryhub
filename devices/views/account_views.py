...
def session_ping(request):
    # Force the session to be saved, which updates the expiry timestamp
    request.session.modified = True 
    
    # Return the remaining time (optional, but good for debugging)
    expiry = request.session.get_expiry_age()
    
    return JsonResponse({'status': 'active', 'timeout': expiry})
